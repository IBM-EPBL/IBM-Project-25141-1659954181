...
@app.route('/getApplications', methods =["GET","POST"])
def getApplications():
    try:
        if request.method=="GET":
            ...
            sql1="SELECT * FROM  JOBS WHERE COMPANY=?"
            ...
            while dictionary != False:
                ...
                inst['POSTEDDATE']=dictionary['POSTEDDATE']
                jobid=inst['JOBID']
                sqlForApplication="SELECT * FROM  APPLICATIONS WHERE JOBID=?"
                stmtInner = ibm_db.prepare(conn, sqlForApplication)
                ibm_db.bind_param(stmtInner, 1, jobid)
                ibm_db.execute(stmtInner)
                application = ibm_db.fetch_both(stmtInner) 
                print(inst)
                print("=====================================================")
                while( application!=False):
                    print(application["FIRSTNAME"],application["LASTNAME"],application["EMAILID"])
                    application=ibm_db.fetch_both(stmtInner) 
                print("====================================================")
                dictionary = ibm_db.fetch_both(stmt) 
